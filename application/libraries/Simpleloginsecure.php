...
class SimpleLoginSecure
{
	 ...
	 * @return	bool
	 */
	function create($user_email = '', $user_pass = '', $user_role = 0, $auto_login = true) 
	{
		...
		$user_pass_hashed = $this->hash_password($user_pass);
	 ...
	 * @return	bool
	 */
	function login($user_email = '', $user_pass = '', $debug = FALSE) 
	{	
		...
		//Check against user table
		$this->CI->db->where('LCASE(user_email)', strtolower($user_email)); 
		...
		if ($query->num_rows() > 0){
			...
			//RETAIN REDIRECT URL
			if($this->CI->session->userdata('return_url') != ''){
				$return_url = $this->CI->session->userdata('return_url');
			}
			...
			$user_data['type'] = $this->user_table;
			unset($user_data['user_pass']);
			if(isset($return_url)){
				$this->CI->session->set_userdata('return_url', $return_url);
			}
	...
	function reset_user_session(){
		$this->CI =& get_instance();
		$sql = "SELECT * FROM users WHERE user_id = ? AND active = 1";
		$query = $this->CI->db->query($sql, array($this->CI->session->userdata('user_id')) );
		$user_data = $query->row_array();
		$user_data['logged_in'] = true;
		unset($user_data['user_pass']);
		$this->CI->session->set_userdata($user_data);
	...
	function hash_password($user_pass){
		...
		$hasher = new PasswordHash(PHPASS_HASH_STRENGTH, PHPASS_HASH_PORTABLE);
		return $hasher->HashPassword($user_pass);
	}
